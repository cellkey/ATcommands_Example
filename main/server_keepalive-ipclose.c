...
/** Keepalive string sent to server (in flash). */
static const char KEEP_ALIVE[] = "1\r\nA\r\n";
/** OPENED ack after APPROVED: 11 bytes */
static const char OPENED_ACK[] = "6\r\nOPENED\r\n";
#define OPENED_ACK_LEN 11

#define KEEP_ALIVE_LEN  (sizeof(KEEP_ALIVE) - 1)   /* 6 bytes */
...
void server_handle_incoming_line(const char *line) {
    ...
    if (s_saw_ka_one && (strcmp(line, "A") == 0 || strstr(line, "A\r\n") != NULL)) {
        ...
        ESP_LOGI(TAG, "KA ACK from server (1/A sequence)");
        return;
    }
...
void server_on_ipclose(int link_id) {
    ...
    ESP_LOGI(TAG, "+IPCLOSE: link %d (server disconnected), scheduling reconnect", link_id);
...
static void server_keepalive_task(void *arg) {
    (void)arg;
    ESP_LOGI(TAG, "Keepalive task started: first KA in %d s, then every %d s", SERVER_KA_FIRST_DELAY_SEC, SERVER_KA_INTERVAL_SEC);

    /* First KA 8 s after connect */
    vTaskDelay(pdMS_TO_TICKS((uint32_t)SERVER_KA_FIRST_DELAY_SEC * 1000));
    if (!s_ka_task_running) {
        goto exit_task;
    }
    /* Send first KA immediately after initial delay */
    if (send_at_then_raw_data(SERVER_TCP_LINK_ID, (int)KEEP_ALIVE_LEN, (const uint8_t *)KEEP_ALIVE) == AT_RESULT_SUCCESS) {
        ESP_LOGI(TAG, "KEEP_ALIVE sent (%d bytes): \"1\\r\\nA\\r\\n\"", (int)KEEP_ALIVE_LEN);
        s_ka_sent_waiting_ack = true;
    } else {
        ESP_LOGW(TAG, "KA send failed (first)");
    }

    while (s_ka_task_running) {
        /* Wake on +CIPRXGET: 1,1 (pending read), +CLCC (pending ring), or after 45 s for next KA */
        uint32_t n = ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS((uint32_t)SERVER_KA_INTERVAL_SEC * 1000));
        ...
        if (n > 0 && s_pending_ipclose_reconnect) {
            ...
            s_ka_sent_waiting_ack = false;
            s_ka_retry_pending = false;
            ESP_LOGI(TAG, "Reconnecting after +IPCLOSE (server link closed)");
