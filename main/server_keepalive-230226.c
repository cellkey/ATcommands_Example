...
static void server_keepalive_task(void *arg) {
    ...
    if (send_at_then_raw_data(SERVER_TCP_LINK_ID, (int)KEEP_ALIVE_LEN, (const uint8_t *)KEEP_ALIVE) == AT_RESULT_SUCCESS) {
        ...
        ESP_LOGI(TAG, "KEEP_ALIVE actual bytes (hex): %02X %02X %02X %02X %02X %02X",
                 (unsigned)(unsigned char)KEEP_ALIVE[0], (unsigned)(unsigned char)KEEP_ALIVE[1],
                 (unsigned)(unsigned char)KEEP_ALIVE[2], (unsigned)(unsigned char)KEEP_ALIVE[3],
                 (unsigned)(unsigned char)KEEP_ALIVE[4], (unsigned)(unsigned char)KEEP_ALIVE[5]);
